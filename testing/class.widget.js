class CWidgetItemNavigatorRME extends CWidget {
	...
	setContents(response) {
		...
		if (this.#selected_group_identifier !== null) {
			const selector = `[data-group_identifier='${JSON.stringify(this.#selected_group_identifier)}']`;
			const node = this._container.querySelector(selector);

			if (node) {
				this.#selected_itemid_group = node.getAttribute('data-ids');
	...
	initAutocomplete() {
				...
				groupIdentifiers.forEach(group => {
					if (searchRegex.test(group.toLowerCase())) {
						...
						$item.on('click', function() {
							$inputBox.val(group);
							$dropdown.hide();
							const $groupNode = $itemNavigator.find(`[data-group_identifier='["${group.split(': ').join('","')}"]']`);
		...
		function processSelectedNode(groupNode) {
			if (groupNode.length > 0) {
				const identifier = JSON.parse(groupNode.attr('data-group_identifier'));
				self.#selected_group_identifier = identifier;

				const descendantIds = groupNode[0].getAttribute('data-ids');
